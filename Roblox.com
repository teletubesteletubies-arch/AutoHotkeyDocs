...
<ul>
  <li>In Notepad (or a <a href="misc/Editors.htm">text editor</a> of your choice), save a file with the <code class="no-highlight">.ahk</code> filename extension. On some systems you may need to enclose the name in quotes to ensure the editor does not add another extension (such as .txt).
  ...
  <li>In Explorer, right-click in empty space in the folder where you want to save the script, then select <strong>New</strong> and <strong>AutoHotkey Script</strong>. You can then type a name for the script (taking care not to erase the <code class="no-highlight">.ahk</code> extension if it is visible).</li>
  <li>In the <a href="#dash">Dash</a>, select <a href="#newscript">New script</a>, type a name for the script (excluding the <code class="no-highlight">.ahk</code> extension) and click <em>Create</em> or <em>Edit</em>. The template used to create the script and the location it will be saved can be configured within this window, and set as default if desired.</li>
</ul>
...
<ul>
  ...
  <li>Only syntax is checked, not semantics. For instance, <code class="no-highlight">xyz,</code> is invalid in v2, so is assumed to be a valid v1 command. <code class="no-highlight">xyz 1</code> could be a function statement in v2, but is assumed to also be a valid v1 command, and is therefore ignored.</li>
...
<p>For example, <code class="no-highlight">_H, 64, !ANSI</code> would prefer AutoHotkey_H if available, 64-bit if compatible with the system, and finally Unicode over ANSI.</p>
...
<table class="info">
  ...
  <tr id="SlashRunWith">
    ...
    <td>Specifies additional <a href="#launcher-criteria">criteria</a> for determining which executable to use to launch the script. For example, <code class="no-highlight">/RunWith UIA</code>.</td>
  </tr>
  <tr id="SlashWhich">
    <td>/Which</td>
    <td>
      ...
      <p>Stdout receives the following UTF-8 strings, each terminated with <code class="no-highlight">`n</code>:</p>
      <ul>
        ...
        <li>Any additional command-line switches that the launcher would insert, such as <code class="no-highlight">/CP65001</code>.</li>
      </ul>
...
<ul>
  ...
  <li>Hide template: Adds the template name to a list of templates that will not be shown in the GUI. To unhide a template, delete the corresponding registry value from <code class="no-highlight">HKCU\Software\AutoHotkey\New\HideTemplate</code>.</li>
...
*/</pre>
<p>If the INI section starts with <code class="no-highlight">/*</code> and ends with <code class="no-highlight">*/</code> as shown above, it is not included in the created file.</p>
...
<ol>
  ...
  <li>Install v1 as an additional version. Running a v1.1.34.03 or later installer gives this option. Alternatively, use the <code class="no-highlight">/install</code> switch described below. Each version installs into its own subdirectory.</li>
</ol>
...
<p>To directly install to the <em>DESTINATION</em> directory, use <code class="no-highlight">/installto</code> or <code class="no-highlight">/to</code> (the two switches are interchangeable) as shown below, from within the source directory. Use either a downloaded setup.exe or files extracted from a downloaded zip or other source.</p>
...
<p>The full command string for the above is registered as <em>InstallCommand</em> under <code class="no-highlight">HKLM\Software\AutoHotkey</code> or <code class="no-highlight">HKCU\Software\AutoHotkey</code>, with <code class="no-highlight">%1</code> as the substitute for the source directory. Using this registry value may be more future-proof.</p>
...
<p>Use the <code class="no-highlight">/silent</code> switch to suppress warning or confirmation dialogs and prevent the <a href="#dash">Dash</a> from being shown when installation is complete. The following actions may be taken automatically, without warning:</p>
<ul>
